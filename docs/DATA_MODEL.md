...
İlgili: [architecture.md](architecture.md) · [adr/](adr/) ·
[SEMA_VE_URUN_YOL_HARITASI.md](SEMA_VE_URUN_YOL_HARITASI.md) · [KVKK.md](KVKK.md)
...
| Tablo | Ne tutar | Notlar |
|---|---|---|
| `users` | Hesaplar | `university_id` **nullable** = platform hesabı (tenant'sız çalışan). E-posta tenant başına tekil + `email = lower(email)` CHECK. `deleted_at` = anonimleştirilmiş (bkz. [KVKK.md](KVKK.md)) |
...
| Eksik | Nerede |
|---|---|
| Etkinlikler, kayıt ve yoklama | [3.1](SEMA_VE_URUN_YOL_HARITASI.md) — en büyük alan eksiği |
...
| Oturum/token iptali | [GUVENLIK_YOL_HARITASI.md](GUVENLIK_YOL_HARITASI.md) Tier 1.3 |
