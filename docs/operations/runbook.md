...
> [machine-setup.md](machine-setup.md) (network/Caddy/two-machine layout),
...
Caddy reverse proxy, `.test` DNS, `BIND_ADDR`, certificate trust and two-machine
layout are **machine-specific** — see
[machine-setup.md](machine-setup.md) (sections "Laptop'ta BIND_ADDR" and
"Sorun giderme").

At a glance: `https://uniclub.test` → production, `https://test.uniclub.test` →
dev. Nothing is internet-facing without port forwarding.
...
1. **Confirm.** Check `/health`, then **Grafana** (`:3001`) — logs (Loki) and
   ...
   the exact server log line and stack trace. See [logging.md](logging.md).
